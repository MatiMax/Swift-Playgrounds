/**
 Name:      QR Code.playground
 Purpose:   Generate a QR code from a text string and copy it to the clipboard, adapted for platform independance.
...
//: We use a `@State` variable to track the encoded message and a `@GestureState` variable for the scale of the QR code bitmap.
...
//: This `Image` displays the actual QR code which gets generated by the `qrCodeGenerated` function.
            #if os(macOS)
            Image(nsImage: qrCodeGenerator().image as! NSImage) //: Forced downcast to NSImage, see explanation in `qrGenerator()`
            ...
            #elseif os(iOS)
            Image(uiImage: qrCodeGenerator().image as! UIImage) //: Forced downcast to NSImage, see explanation in `qrGenerator()
                .interpolation(.none) //: Turn off anti-aliasing when scaling up or down
                .resizable()          //: The image can be resized
                .scaledToFit()        //: Always scale the image to fit the frame
...
/*:
 And this does not work, either:
     ...
     UIPasteboard.general.image = UIImage(cgImage: self.qrCodeGenerator().uiImage.cgImage!, scale: self.scaleQR * 8, orientation: .up)
 */
                    }
            }
...
/*:
 ...
 In order to use the QR code in SwiftUI's `Image` view we first need to generate a `CGImage` representation and then create a `UIImage` or an `NSImage` of it which is returned within a tuple.
     
 The function returns a tuple of both an `NSImage` or `UIImage` and a `CIImage`, because only `CIImage` supports scaling without applying any interploation or anti-aliasing so that it can be used as source for placing it onto the clipboard.
 
 The only *ugly* thing with this function is that we have to return the base-class of `NSImage` and `UIImage` which is an `NSObject`  in order to get the code work platform-independently. As a result the `NSObject` must be forced to downcast which is possible, but again — *ugly*. We use this ugly method instead of duplicating the method to provide two different implementations of argument lists — which would certainly be the cleaner way:

     func qrCodeGenerator() -> (nsImage: NSImage, ciImage: CIImage?) { … }
     func qrCodeGenerator() -> (uiImage: NSImage, ciImage: CIImage?) { … }
     
 Wouldn't it be nice if we could implement a function which is capable of returning different types depending on the platform it compiles to? 😃
 */
    func qrCodeGenerator() -> (image: NSObject, ciImage: CIImage?) {
        ...
        if let outputImage = qrCode.outputImage {
            if let cgImg = context.createCGImage(outputImage, from: outputImage.extent) {
                ...
                #else
                    return (UIImage(cgImage: cgImg), outputImage)
                #endif
            } else {
